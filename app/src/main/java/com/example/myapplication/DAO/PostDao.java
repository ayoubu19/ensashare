...
public class PostDao {
    ...
    public PostDao(String group) {
        this.group = group;
        databaseReference = database.getReference().child("groups").child("group:"+group).child("posts").orderByKey().getRef();
    }
                ...
                if (postsObject != null) {
                    ...
                    for (String key : keys) {
                        ...
                        if (p.getTypeFile().equals("pdf")) {
                            Toast.makeText(context, p.getContent(), Toast.LENGTH_LONG).show();
                            posts.add(p);
                        }
                    }
                }
